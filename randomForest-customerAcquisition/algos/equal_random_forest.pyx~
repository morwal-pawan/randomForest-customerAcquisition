...
cdef double* bag_tree(long sampleSize,long dataLen,long height,long *features,long n_features) nogil:
	...
	cdef long nopos
	cdef long noneg
	cdef long half_samples
	cdef long *pos_samples=<long *>malloc(sizeof(long)*dataLen)
	cdef long *neg_samples=<long *>malloc(sizeof(long)*dataLen)
	#with gil:
	tttime=time(NULL)
	srand(tttime)
	#sample_map[-1]=-1
	...
	#half_samples=sampleSize/2
	half_samples=<long>(0.25*sampleSize)
	
	for k in range(half_samples):
		ttemp=rand()%nopos
		ttemp=pos_samples[ttemp]
		if sample_map.find(ttemp)!=sample_map.end():
			sample_map[ttemp]=sample_map[ttemp]+1
		else:
			sample_map[ttemp]=1

	for k in range(half_samples,sampleSize):
		ttemp=rand()%noneg
		ttemp=neg_samples[ttemp]
		if sample_map.find(ttemp)!=sample_map.end():
			sample_map[ttemp]=sample_map[ttemp]+1
		else:
			sample_map[ttemp]=1
...
probs=bagging_trees(dataLen,500,dataLen,15,feature_i,num_features,testLen,6)



outfile = open('./results/equal_random_forest', "wb")
outfile.write('id,repeatProbability\n')
