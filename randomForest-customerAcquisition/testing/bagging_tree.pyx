...
sum_probs=<double *>malloc(sizeof(double)*testLen)
...
for t in range(60):
	#now probs is sum over next 50 trees
	probs=bagging_trees(dataLen,50,dataLen,15,feature_i,num_features,testLen,8)
	test_prob=[]
	for k in range(testLen):
		sum_probs[k]+=probs[k]
		test_prob.append(sum_probs[k]/<double>((t+1)*50))
...
plt.ylabel('AUC')
plt.title('AUC vs Iterations '+'Max AUC @ '+str(iterr))
#plt.show()
plt.savefig('foo.png')
...
probs=bagging_trees(160057,600,dataLen,15,feature_i,num_features,8)

outfile = open('./new_mem_opti_home_made_bagging_trees', "wb")
outfile.write('id,repeatProbability\n')
for k in range(testLen):
